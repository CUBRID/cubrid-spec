%define cubrid_version 8.4.1
%define build_version  2032
...
install -c -m 644 contrib/scripts/cubrid.sh %{buildroot}%{_sysconfdir}/profile.d/cubrid.sh
install -c -m 644 contrib/scripts/cubrid.csh %{buildroot}%{_sysconfdir}/profile.d/cubrid.csh
...
chrpath -d %{buildroot}%{_bindir}/cub_ftproc
...
. %{_sysconfdir}/profile.d/cubrid.sh
%{_prefix}/share/cubrid/demo/make_cubrid_demo.sh > /dev/null 2>&1
chown %{cubrid_user}:%{cubrid_user} -R %{cubridvardir}/databases/demodb > /dev/null 2>&1
cubrid server start demodb > /dev/null 2>&1
...
%postun demodb 
/sbin/ldconfig
service cubrid stopdb demodb > /dev/null 2>&1
service cubrid deletedb demodb > /dev/null 2>&1
